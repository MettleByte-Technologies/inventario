        ...
        switch (objResulSet["error"])
        {
            case 0:
                ...
                $.each( row, function( key, value ) {
                  console.log( value );
                  var rowNode = table
                     .row.add( [ value["DEPE_CODIGO_PRODUCTO"], value["DESCRIPCION"], 0, parseFloat(value["DEPE_PRECIO"]).toFixed(2), value["DEPE_CANTIDAD"], parseFloat(value["DEPE_PRECIO_LISTA"]).toFixed(2), value["DEPE_FECHA_ENTREGA"], ''])
                     .draw()
                     .node();
               });           
