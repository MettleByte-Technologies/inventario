      ...
      if (parseFloat(qty) > 0.0) {
        ...
        if (aniadir == 1) {
          ...
          $.ajax({
            ...
            type: "POST",
            data: {
              product: product, //row.children()[0].innerText,
            ...
            success: function (resulSet) {
              ...
              switch (objResulSet["error"]) {
                case 0:
                  ...
                  console.log(
                    "selected ==> " +
                      list_code_tmp.substr(0, list_code_tmp.length - 1)
                  );
                  ...
                  var rowNode = table.row //.row.add( [ row.children()[0].innerText, row.children()[1].innerText, stock_saldo, parseFloat(price).toFixed(2), '<input type="text" name="items" id="items" CssClass="txt-observ" value="' + qty + '"/>', parseFloat(dscto).toFixed(2), parseFloat(total).toFixed(2), fecha, '<input type="text" name="observs" id="observs" CssClass="txt-observ" value=""/>',porcDscto] )
                    .add([
                      product,
                      productName,
                      stock_saldo,
                      parseFloat(price).toFixed(2),
                      '<input type="text" name="items" id="items" CssClass="txt-observ" value="' +
                        qty +
                        '"/>',
                      parseFloat(dscto).toFixed(2),
                  ...
                  console.log(
                    total_items + " - " + total_dscto + " - " + total_pagar
                  );
                  ...
                  $.ajax({
                    ...
                    data: {
                      ...
                      notaventa: "0",
                      bodega: document.getElementById("select-bodega").value,
                      //                      $("#select-bodega").find("option:selected").val(),
                    },
                    success: function (resulSet) {
        ...
        success: function (resulSet) {
          ...
          switch (objResulSet["error"]) {
            case 0:
              ...
              //console.log((parseFloat(qty)*parseFloat(row.children()[3].innerText)*parseFloat(porcDscto)/100).toFixed(2));
              var dscto =
                (parseFloat(qty) * parseFloat(price) * parseFloat(porcDscto)) /
                100; //.toFixed(2);
              //console.log("%: " + dscto);
              var subtotal = parseFloat(qty) * parseFloat(price);
              var total = subtotal - dscto; //.toFixed(2);
              ...
              var rowNode = table.row //.row.add( [ row[0], row[1], '<input type="text" name="item_"' + row[0] + ' id="item_"' + row[0] + ' CssClass="txt-observ" value="' +  + '"/>', parseFloat(price).toFixed(2), qty, parseFloat(dscto).toFixed(2), parseFloat(total).toFixed(2), fecha, '<input type="text" name="txt-observ" id="txt-observ" CssClass="txt-observ" value="0"/>'] )
                .add([
                  row[1],
                  row[2],
                  stock_saldo,
