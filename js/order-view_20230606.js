...
$(document).ready(function() {
   ...
   $("#stock-alert").hide();
    ...
    setTableInit(!($("input[name='data-estado']").val()=="V"));    
   
    //$(".txt-observ").editable("save.php");

    function setTableInit(estado)
    {
                      ...
                      action: function ( e, dt, node, config ) {
                        ...
                        $.each(rows1, function( index, value ) {
                           ...
                           if (newPerc.length > 0){
                              ...
                              order_det = order_det + '{"newperc":"'+newPerc+'", "product":"'+value[0]+'"}';                          
                              console.log('{"newperc":"'+newPerc+'", "product":"'+value[0]+'"}');  
                           }
                        });
 ...
 /************************************ */  
                   ...
                   },
                   {
                     ...
                     action: function ( e, dt, node, config ) {
                        ...
                        $.ajax({
                           ...
                           type:'POST',
                           data: {action:"reprove-order",
                                 vend_codigo:$("input[name='id_vendedor']").val(),
                                 vend_empresa:$("input[name='id_empresa']").val(),
                                 nOrder: $("input[name='nOrder']").val(),
                                 observ: $("input[name='pedi-observ']").val(),                                
                                 deuda: ($("input[name='deuda']").prop("checked") == true  ? "F" : "D")
                              },
                  ...
                  },                    
                   {
                     ...
                     action: function ( e, dt, node, config ) {
                        ...
                        let fieldExport = [['vendedor', parseInt($("input[name='vend-id']").val())], ['Cliente', parseInt($("#client-id").val()), $("#client-name").val()], ['',''], ['PRODUCTO','CANTIDAD']]    

                        $.ajax({
                           url:'../business/order.php', 
                           type:'POST',
                           data: {action:"export-file-order",
                  ...
                  },
                  {
                    ...
                    action: function ( e, dt, node, config ) {
                       ...
                       let fieldExport = [['vendedor', parseInt($("input[name='vend-id']").val())], ['Cliente', parseInt($("#client-id").val()), $("#client-name").val()], ['',''], ['PRODUCTO','CANTIDAD']]
                       let oracledExport = 'INSERT INTO FA_PEDIDO(PEDI_CODIGO_CLIENTE, PEDI_CODIGO_VENDEDOR) VALUES('+ parseInt($("input[name='vend-id']").val()) + ', "' + $("#client-name").val() + '"); \n';
                       ...
                       for (let i = 0; i < rows.length; i++) {
                          ...
                          qty = rows[i][3];
                          fieldExport.push([prod, qty]);
                          oracledExport = oracledExport + 'INSERT INTO FA_DETALLE_PEDIDO(DEPE_CODIGO_PRODUCTO, DEPE_CANTIDAD) VALUES('+ prod + ', "' + qty + '"); \n';
                        }                 
                       ...
                       $.ajax({
                           url:'../business/order.php',
                           type:'POST',
                           data: {action:"export-order-review",
                               vend_codigo:$("input[name='vend-id']").val(),
                               vend_empresa:$("input[name='id_empresa']").val(),
                               nOrder: $("input[name='nOrder']").val()},
                           success: function(resulSet) {
                             ...
                             console.log(resulSet);
                               ...
                               switch (objResulSet["error"])
                               {
                                   case 0:
                                    //ExportToExcel(fieldExport, $("#nOrder").text(),false);
                                    guardarArchivoDeTexto(oracledExport, $("input[name='nOrder']").val() + ".txt");
        ...
        if (dataNotaVenta == '1')  $("input[name='notaventa']").prop("checked", true);
   ...
   $("#save-items").click(function(e){
      ...
      rows.children().each(function(e){
       ...
       let order_head = '[{"tot_qty":'+$("#total-items").text()+', "tot_dscto":'+$("#total-dscto").text()+', "total":'+$("#total-pagar").text()+', "observ": "'+$("input[name='pedi-observ']").val()+'", "notaventa": "'+($("input[name='notaventa']").prop("checked") == true ? '1' : '0')+'"}]';
   ...
   /*$("#save-order").click(function(e){
    ...
    function ExportToExcel(ws_data, fn, dl) {
      ...
      saveAs(new Blob([s2ab(wbout)],{type:"application/octet-stream"}), 'pedido.xlsx');
