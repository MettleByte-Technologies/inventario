             ...
             buttons: [
                   {
                      ...
                      action: function ( e, dt, node, config ) {
                        ...
                        $.each(rows1, function( index, value ) {
                            ...
                            order_det = order_det + '{"newqty":"'+newQty+'", "newperc":"'+newPerc+'", "qty":'+newQty+', "dscto":'+dscto+', "porcdscto":'+porcDscto+', "valor":'+total+', "product":"'+value[0]+'"}';                          
                            //let order_det = '[{"product":'+product+', "qty":'+qty+', "valor":'+total+', "dscto":'+dscto+',"price":'+price+',"porcdscto":'+porcDscto+'}]';
                            console.log('{"newqty":"'+newQty+'", "newperc":"'+newPerc+'", "qty":'+newQty+', "dscto":'+dscto+', "porcdscto":'+porcDscto+', "valor":'+total+', "product":"'+value[0]+'"}');
                             ...
                             success: function(resulSet) {
                                 ...
                                 switch (objResulSet["error"])
                                 {
                                     case 0:
                                         ...
                                         if (nitems > 1)
                                             $("#message").html("Items " + itemsList + " fueron eliminado");
                                         else
                                             $("#message").html("Item " + itemsList + " fue eliminado");
        ...
        switch (objResulSet["error"])
        {
            case 0:
                ...
                $.each( row, function( key, value ) {
                    ...
                    var rowNode = table
                        //.row.add( [ value["DEPE_CODIGO_PRODUCTO"], value["DESCRIPCION"], 0, parseFloat(value["DEPE_PRECIO"]).toFixed(2), value["DEPE_CANTIDAD"], parseFloat(value["DEPE_COSTO"]).toFixed(2), parseFloat(value["DEPE_PRECIO_LISTA"]).toFixed(2), value["DEPE_FECHA_ENTREGA"], '<input type="text" name="txt-observ" id="txt-observ" CssClass="txt-observ" value="'+ value["DEPE_CARACTER"] +'"/>'  ])
                        .row.add( [ value["DEPE_CODIGO_PRODUCTO"], value["DESCRIPCION"], value["SALDO_INV"], parseFloat(value["DEPE_PRECIO"]).toFixed(2), '<input type="text" name="items" id="item_' + value["DEPE_CODIGO_PRODUCTO"] + '" CssClass="txt-observ" value="' + value["DEPE_CANTIDAD"] + '"/>', parseFloat(value["DEPE_COSTO"]).toFixed(2), parseFloat(value["DEPE_PRECIO_LISTA"]).toFixed(2), value["DEPE_FECHA_ENTREGA"], '<input type="text" name="observs" id="observs" CssClass="txt-observ" value="'+ value["DEPE_CARACTER"] +'"/>', value["DEPE_PORC_DSCTO1"]] )
                        .draw()
                        .node();
